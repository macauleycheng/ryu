...
class app_example(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        #set outter tag
        print "table 10 set outter tag"        
        actions = [parser.OFPActionPopVlan(), 
                   parser.OFPActionSetField(ofdpa_ovid=0x101e)]
        inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS, actions),
                parser.OFPInstructionGotoTable(11)]
        match = parser.OFPMatch()
        match.set_vlan_vid_masked(0x101e, 0x1fff)
        match.set_in_port(2)    
        self.add_flow(datapath, 10 ,1, match , inst)       

        #match outter tag       
        print "table 11 match outter tag"        
        actions = []
        inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS, actions),
                parser.OFPInstructionGotoTable(20)]
        match = parser.OFPMatch(ofdpa_ovid=0x101e, in_port=2, vlan_vid=(0x1001, 0x1fff))
        self.add_flow(datapath ,11 ,1, match , inst)
